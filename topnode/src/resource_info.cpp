  ...
  const char * default_result = nullptr)
{
  ...
  if (!f) {
    ...
    throw std::runtime_error("Failed to read from file " + path.string());
  }
  std::string line;
...
get_memory_usage(const std::filesystem::path & proc_root)
{
  ...
  auto memory_state = get_memory_state(proc_root);
  result.shared_size = (memory_state.shared_page_count * getpagesize()) >> 10;
  result.virtual_size = (memory_state.total_program_size * getpagesize()) >> 10;
  ...
  LastTickTimes & last_tick_times)
{
  ...
  auto now = clock->now();
  result.elapsed_time = (now - last_tick_times.last_measure_time).nanoseconds();
  ...
  last_tick_times.last_measure_time = now;

  result.load_average = get_load_average(proc_root);
